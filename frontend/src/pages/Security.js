...
import { Shield, Ban, Smartphone, MapPin, Trash2, Plus, AlertTriangle, CheckCircle, XCircle, Monitor } from 'lucide-react';
    ...
    <div className="space-y-6">
                ...
                <Table>
                  <TableHeader>
                    <TableRow className=\"border-slate-700 hover:bg-slate-800/50\">
                      ...
                      <TableHead className=\"text-slate-300\">{t('blocked_by')}</TableHead>
                  ...
                  <TableBody>
                    {blockedIPs.length === 0 ? (
                      ...
                      blockedIPs.map((ip) => (
                        <TableRow key={ip.id} className=\"border-slate-700 hover:bg-slate-800/30\">
                          ...
                          <TableCell className=\"text-slate-400 text-sm\">{ip.blocked_by || t('system')}</TableCell>
        ...
        <TabsContent value=\"devices\" className=\"space-y-4\">
          <Card className=\"glass border-slate-700\">
            <CardHeader>
              ...
              <CardDescription className=\"text-slate-400\">
                Активные сессии устройств пользователей
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className=\"overflow-x-auto\">
                <Table>
                  ...
                  <TableBody>
                    {devices.length === 0 ? (
                      ...
                      devices.map((device) => (
                        <TableRow key={device.id} className=\"border-slate-700 hover:bg-slate-800/30\">
                          <TableCell className=\"text-white\">{device.user_id.slice(0, 8)}...</TableCell>
                          <TableCell>
                            <div className=\"flex items-center gap-2\">
                              <Smartphone className=\"w-4 h-4 text-cyan-400\" />
                              <span className=\"text-slate-300\">{device.device_name}</span>
                            </div>
                          </TableCell>
                  ...
                  <TableBody>
                    {loginAttempts.length === 0 ? (
                      ...
                      loginAttempts.map((attempt) => (
                        <TableRow key={attempt.id} className=\"border-slate-700 hover:bg-slate-800/30\">
      ...
      <Dialog open={showDeviceMap} onOpenChange={setShowDeviceMap}>
        <DialogContent className=\"glass border-slate-700 max-w-2xl\">
          <DialogHeader>
            <DialogTitle className=\"text-white\">GPS Локация устройства</DialogTitle>
            <DialogDescription className=\"text-slate-400\">
              ...
              <div className=\"space-y-4\">
                <div className=\"bg-slate-800 p-4 rounded-lg space-y-2\">
                  ...
                  <div className=\"flex justify-between\">
                    ...
                    <span className=\"text-white text-sm\">{formatDate(selectedDevice.gps_updated_at)}</span>
                  </div>
                </div>
                <div className=\"bg-slate-800 rounded-lg overflow-hidden\" style={{ height: '300px' }}>
                  <iframe
                    width=\"100%\"
                    height=\"100%\"
                    frameBorder=\"0\"
                    style={{ border: 0 }}
                    src={`https://www.openstreetmap.org/export/embed.html?bbox=${selectedDevice.gps_longitude - 0.01},${selectedDevice.gps_latitude - 0.01},${selectedDevice.gps_longitude + 0.01},${selectedDevice.gps_latitude + 0.01}&layer=mapnik&marker=${selectedDevice.gps_latitude},${selectedDevice.gps_longitude}`}
                    title=\"GPS Location\"
                  />
                </div>
                <a
                  href={`https://www.openstreetmap.org/?mlat=${selectedDevice.gps_latitude}&mlon=${selectedDevice.gps_longitude}#map=15/${selectedDevice.gps_latitude}/${selectedDevice.gps_longitude}`}
                  target=\"_blank\"
