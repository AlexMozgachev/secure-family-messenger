...
# =============================================================================
# Secure Messenger Builder - Исправленная установка для Ubuntu 22.04 LTS
# Фиксы: IPv4 only, без SSL по умолчанию, улучшенная обработка ошибок
# =============================================================================
...
# Получение IP адреса с приоритетом IPv4
get_server_ip() {
    print_status "Определение IP адреса сервера..."
    
    # Опции для пользователя
    echo ""
    echo "Выберите тип подключения:"
    echo "1) Использовать localhost (127.0.0.1) - рекомендуется для тестов"
    ...
    case $IP_CHOICE in
        1)
            SERVER_IP="127.0.0.1"
            print_success "Использован localhost: $SERVER_IP"
            ;;
        2)
            # Приоритет IPv4 адресов
            ...
            if [ -z "$SERVER_IP" ] || [[ "$SERVER_IP" =~ ":" ]]; then
                print_warning "Не удалось получить IPv4 адрес, используем localhost"
                SERVER_IP="127.0.0.1"
            else
                print_success "Получен IPv4 адрес: $SERVER_IP"
            fi
            ;;
        3)
            read -p "Введите IP адрес: " SERVER_IP
            if [ -z "$SERVER_IP" ]; then
                SERVER_IP="127.0.0.1"
                print_warning "Пустой IP, используем localhost: $SERVER_IP"
            else
                print_success "Установлен IP: $SERVER_IP"
            fi
            ;;
        *)
            SERVER_IP="127.0.0.1"
            print_warning "Неверный выбор, используем localhost: $SERVER_IP"
            ;;
    esac
}
...
print_intro() {
    clear
    echo -e "${CYAN}================================================================${NC}"
    echo -e "${CYAN}    🚀 Secure Messenger Builder - Ubuntu 22.04 LTS (Fixed)    ${NC}"
    echo -e "${CYAN}================================================================${NC}"
    echo -e "${GREEN}🔧 Исправления: IPv4 only + без SSL по умолчанию${NC}"
...
update_system() {
    ...
    # Установка базовых пакетов (без SSL утилит по умолчанию)
    apt install -y \
        curl \
        wget \
        git \
...
EOF
    ...
    deactivate
    print_success "Backend настроен (FastAPI + Python 3.10) без SSL"
}
...
verify_installation() {
    ...
    # Создание тестового пользователя админ
    print_status "Создание тестового админа..."
    cd /opt/Secure-Messenger-Builder/backend
    source venv/bin/activate
    python3 -c "
import asyncio
...
" 2>/dev/null || echo "Админ уже создан или ошибка создания"
    deactivate
}
...
show_final_info() {
    ...
    echo -e "   • Веб-интерфейс:    ${GREEN}http://$SERVER_IP:3000${NC}"
    ...
    echo "   1. SSL отключен для упрощения установки"
    echo "   2. Смените пароль admin после входа"
    echo "   3. Настройте SSL для production"
    echo ""
